#include "TabularRandomSearch.hpp"	// The .cpp file for an class should include the header file (the .hpp file).
...
TabularRandomSearch::TabularRandomSearch(const int& stateDim, const int& numActions, const double & gamma, const int & N, const int & maxEps) : EpisodicAgent(N)
{
...
int TabularRandomSearch::getAction(const Eigen::VectorXd& s, std::mt19937_64& generator) const
{
...
void TabularRandomSearch::reset(std::mt19937_64& generator)
{
...
void TabularRandomSearch::episodicUpdate(mt19937_64 & generator)
{
	...
	if (epCount < (int)(maxEps*0.9))
	{
		...
		newTheta = curTheta; // Run the best policy we have found for the last 10% of episodes
}