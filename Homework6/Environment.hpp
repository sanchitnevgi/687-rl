...
public:
	...
	virtual int getMaxEps() const = 0;				// How many episodes should be run?
	virtual int getStateDim() const = 0;			// This function returns the dimension (length) of state vectors.
	virtual int getNumActions() const = 0;			// This function returns |\mathcal A|. Note that we are assuming that the action set is finite.
	virtual double getGamma() const = 0;	// This function returns \gamma
