...
int QLearning::getAction(const Eigen::VectorXd& s, std::mt19937_64& generator)
{
	...
	if (env == 1) {
		...
		actionProbabilities = (w * features).array().exp();
	}
...
{	
	...
	if (env == 1) {
		...
		double delta = r + gamma * maxQ(statePrime) - curQ(state, a);
		...
		double delta = r + (gamma * maxQ(phiPrime)) - w.row(a).dot(phi);
...
double QLearning::maxQ(const int& s)
{
	double result = curQ(s,0);
...
double QLearning::maxQ(const VectorXd& phi)
{
	double result = w.row(0).dot(phi);
	for (int i = 1; i < numActions; i++)
		result = max(result, w.row(i).dot(phi));
	return result;
}
...
VectorXd QLearning::cosine_calculation(const VectorXd& x) {
